    ...
    infer(ctx: Context, expressionType: DataType, isConst: boolean | 0) {
        ...
        if(expressionType.is(ctx, VariantType)) {
            ...
            if(!this.type.is(ctx, VariantConstructorType)) {
                ctx.parser.customError(`Cannot perform pattern matching on non-variant consturctor type ${this.type.getShortName()} against a variant ${expressionType.getShortName()}`, this.location);
            }
            ...
            if(variantConstructorType._parent == null) {
                ctx.parser.customError(`Variant constructor ${variantConstructorType.getShortName()} has no parent`, this.location);
            }
            ...
            if(!res.success) {
                ...
                ctx.parser.customError(`Cannot perform pattern matching on variant constructor ${variantConstructorType.getShortName()} who is not a subtype of a variant ${variantType.getShortName()}: ${res.message}`, this.location);
            }
            // we make sure that the parameters match
            if(variantConstructorType.parameters.length != this.args.length) {
                ctx.parser.customError(`Cannot perform pattern matching on variant constructor ${variantConstructorType.getShortName()} with ${variantConstructorType.parameters.length} parameters against a variant ${variantType.getShortName()} with ${this.args.length} arguments`, this.location);
            }
            ...
            if(!this.type.is(ctx, VariantConstructorType)) {
                ctx.parser.customError(`Cannot perform pattern matching on non-variant consturctor type ${this.type.getShortName()} against a variant constructor ${expressionType.getShortName()}`, this.location);
            }

            // now we make sure that the variant constructor is the same
            let r = matchDataTypes(ctx, this.type, expressionType);
            if(!r.success) {
                ctx.parser.customError(`Variant constructors missmatch, ${expressionType.getShortName()} cannot be matched against ${this.type.getShortName()}`, this.location);
            }

            // we make sure that the parameters match
            if((expressionType as VariantConstructorType).parameters.length != this.args.length) {
                ctx.parser.customError(`Cannot perform pattern matching on variant constructor ${expressionType.getShortName()} with ${(expressionType as VariantConstructorType).parameters.length} parameters against a variant constructor ${this.type.getShortName()} with ${this.args.length} arguments`, this.location);
            }
            ...
            if(!this.type.is(ctx, EnumType)) {
                ctx.parser.customError(`Cannot perform pattern matching on non-enum type ${this.type.getShortName()} against an enum ${expressionType.getShortName()}`, this.location);
            }

            // now we make sure that the enum is the same
            let r = matchDataTypes(ctx, this.type, expressionType);
            if(!r.success) {
                ctx.parser.customError(`Enums missmatch, ${expressionType.getShortName()} cannot be matched against ${this.type.getShortName()}`, this.location);
            }
            
            // enums take no arguments
            if(this.args.length > 0) {
                ctx.parser.customError(`Cannot perform pattern matching on enum type ${this.type.getShortName()} with ${this.args.length} arguments`, this.location);
            }
        }
        else {
            // since we are not using variants at this stage, we make sure we have no arguments
            if(this.args.length > 0) {
                ctx.parser.customError(`Cannot perform pattern matching on non-variant type ${expressionType.getShortName()} with ${this.args.length} arguments`, this.location);
            }

            if(!(expressionType.is(ctx, ClassType) || expressionType.is(ctx, InterfaceType))) {
                ctx.parser.customError(`Cannot perform pattern matching on type ${this.type.getShortName()} against a non-matching type ${expressionType.getShortName()}`, this.location);
            }
            ...
            if(!this.type.is(ctx, InterfaceType)) {
                ctx.parser.customError(`Cannot perform pattern matching on class type ${this.type.getShortName()} against a non-matching type ${expressionType.getShortName()}`, this.location);
            }
        }
    }
    ...
    generateExpression(ctx: Context, baseExpression: Expression): PatternToExpression {
        ...
        if (isEnum) {
            ...
            if(!(this.type instanceof ReferenceType)) {
                ctx.parser.customError(`Cannot perform pattern matching on non-reference type ${this.type.getShortName()}`, this.location);
            }
            ...
            if(referenceType.pkg.length != 2) {
                ctx.parser.customError(`Cannot perform pattern matching on just basic enum type ${this.type.getShortName()}`, this.location);
            }
